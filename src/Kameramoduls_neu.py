...
mhw = None	#Maske für gebogenen Horizont
mhw3 = None
mva = None	#Maske für den Weg des Autos
...
#blob detector für Wände
    wand_params = cv2.SimpleBlobDetector_Params()
    wand_params.filterByArea = True
    wand_params.minArea = 150
    wand_params.maxArea = 50000
    wand_params.filterByCircularity = False
    wand_params.filterByConvexity = False
    wand_params.filterByInertia = False
    ...
    #erstelle blob detector
    wand_detector = cv2.SimpleBlobDetector_create(wand_params)
...
def get_image():
    ...
    image2 = frame[30:240, 0:320]
    #image = cv2.bitwise_and(image2, image2, mask=mhw)
    image = cv2.add(image2, mhw3)
    hoehe = image.shape[0]
...
def get_mask_images():
    ...
    mhw = cv2.imread("/home/pi/FE_2025/Data/Mask_white_top.png")
    mva = cv2.imread("/home/pi/FE_2025/Data/Weg_Maske1.png")
    
    ret, mhw3 = cv2.threshold(mhw, 10, 255, cv2.THRESH_BINARY)	#speichere 3 Kanal Bild
    mhw = cv2.cvtColor(mhw, cv2.COLOR_BGR2GRAY)
    mva = cv2.cvtColor(mva, cv2.COLOR_BGR2GRAY)
    ret, mhw = cv2.threshold(mhw, 10, 255, cv2.THRESH_BINARY)
    mhw = cv2.bitwise_not(mhw)
    ret, mva = cv2.threshold(mva, 10, 255, cv2.THRESH_BINARY)
...
# Betrachte linke und rechte Seite getrennt, nur Ausschnitt
    maskL = mask[0:150, 0:159]
    maskR = mask[0:150, 160:320]
#////////////////////////////////
    crop_mva = mva[hoehe - 150:hoehe, 0:320]
    #mva = wand_filter(crop_mva)
    mva_1 = crop_mva
# Betrachte linke und rechte Seite getrennt, nur Ausschnitt
    mvaL = mva_1[0:150, 0:159]
    mvaR = mva_1[0:150, 160:320]
    
    koll_imgL = cv2.bitwise_and(mvaL, maskL)
    koll_imgR = cv2.bitwise_and(mvaR, maskR)
    
    koll_imgL = cv2.copyMakeBorder(koll_imgL,1,1,1,1, cv2.BORDER_CONSTANT, value=[0,0,0])
    koll_imgR = cv2.copyMakeBorder(koll_imgR,1,1,1,1, cv2.BORDER_CONSTANT, value=[0,0,0])

    koll_imgL = cv2.bitwise_not(koll_imgL)
    koll_imgR = cv2.bitwise_not(koll_imgR)

    koll_imgL = cv2.medianBlur(koll_imgL,5)
    koll_imgR = cv2.medianBlur(koll_imgR,5)
    
    koll_L_keypoints = wand_detector.detect(koll_imgL)
    anzahl_keys = len(koll_L_keypoints)
    if anzahl_keys > 0:
        i = 0
        while i < anzahl_keys:
            s = koll_L_keypoints[i].size
            if s > s_maxL: 
                s_maxL = s
            i = i+1
#/////////////////////////////////////////
    koll_R_keypoints = wand_detector.detect(koll_imgR)
    anzahl_keys = len(koll_R_keypoints)
    if anzahl_keys > 0:
        i = 0
        while i < anzahl_keys:
            s = koll_R_keypoints[i].size
            if s > s_maxR: 
                s_maxR = s
            i = i+1
#/////////////////////////////////////

    if s_maxL > 0 and s_maxL >= s_maxR:
        kollL = True
    
    if s_maxR > 0 and s_maxR >= s_maxL:
        kollR = True
        
    cv2.imshow("histL",maskL)
    cv2.imshow("histR",maskR)
    cv2.imshow("MVA_links",mvaL)
    cv2.imshow("MVA_rechts",mvaR)
    cv2.imshow("Koll_imgR",koll_imgR)
    cv2.imshow("koll_imgL",koll_imgL)
...
#histogramm, um Höhe der Wände zu bestimmen
    ...
    #if maxHistL > 10000 and maxHistR > 10000:
     #   kollL = True
      #  kollR = True
    #else:
     #Seitenkollisionen:
...
#=======change challenge mode here=========
            ...
            while True:
                ...
                cv2.imshow("Maske_MHW",mhw)
                cv2.imshow("Maske_MVA",mva)
