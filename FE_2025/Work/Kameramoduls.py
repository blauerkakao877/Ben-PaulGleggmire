...
def get_colorfilter_data():
    ...
    with open("/home/pi/FE_2025/Data/Colorfilter.dat", "r") as myfile:
...
#histogramm, um Höhe der Wände zu bestimmen
    ...
    # Seitenkollisionen:
    if maxHistL > 12000:  #13000
        kollL = True
    else:
        kollL = False
    
    if maxHistR > 12000:  #13000
        kollR = True
    else:
        kollR = False
...
#histogramm, um Höhe der Wände zu bestimmen
    print("HistL")
    histL = np.sum(maskL, axis=0)
    print("L size: "+str(histL.size))
    print(histL)
    print("HistR")
    histR = np.sum(maskR, axis=0)
    print("R size: "+str(histR.size))
    print(histR)
    print("Finde Ausfahrt")
    while (histL[159 -i] > 0) and (histR[i] >0) and (i < 160):
        i = i+1
        print("i: "+str(i))
...
#----------------------------------------------------
def detect_park_korrektur(hsv_img):
    global blob_detector
    global mask_park
    global crop_img
    
    crop_img = hsv_img[0:50, 0:320]
    mask_park = wand_Magenta_filter(crop_img)
    mask_park = cv2.copyMakeBorder(mask_park,1,1,1,1, cv2.BORDER_CONSTANT, value=[0,0,0])
    mask_park = cv2.bitwise_not(mask_park)
    
    park_keypoints = blob_detector.detect(mask_park)
    anzahl_keys = len(park_keypoints)
    s_max = 0
    x = 0
    y = 0
    if anzahl_keys > 0:
        i = 0
        while i < anzahl_keys:
            s = park_keypoints[i].size
            if s > s_max: 
                x = park_keypoints[i].pt[0]
                y = park_keypoints[i].pt[1]
                s_max = s
            i = i+1
    #pinker blob ist groesser als 3000 Pixel, also korrektur fahren
    if s_max > 3000:
        return True
    else:
        return False
...
#=======change challenge mode here=========
            ...
            while True:
                ...
                detect_park_korrektur(hsv_bild)
                ...
                cv2.imshow("Maske_park",mask_park)
