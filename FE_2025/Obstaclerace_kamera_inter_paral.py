...
#vorrwarts nach links lenken bis 30 Grad vor geradeaus
        ...
        F.vor(0.3)
        alarm_HL, alarm_VL, alarm_HR, alarm_VR = P.prox_alarm()
        while (gesamt > -860) and (alarm_VL == False) and (U.distanz_V() > 5.0):
            time.sleep(0.05)
            winkel, gesamt = G.Winkelmessen()
            alarm_HL, alarm_VL, alarm_HR, alarm_VR = P.prox_alarm()
...
#fertig nach hinten, jetzt gerade vorwarts stellen
        ...
        F.vor(0.3)
        alarm_HL, alarm_VL, alarm_HR, alarm_VR = P.prox_alarm()
        while (gesamt > -890) and (alarm_VL == False) and (U.distanz_V() > 5.0):
            time.sleep(0.05)
            winkel, gesamt = G.Winkelmessen()
            alarm_HL, alarm_VL, alarm_HR, alarm_VR = P.prox_alarm()
        F.stop()
...
#korrigiere rückwaerts
        ...
        F.ruck(0.3)
        alarm_HL, alarm_VL, alarm_HR, alarm_VR = P.prox_alarm()
        while (gesamt > -900) and (alarm_HM == False):
            time.sleep(0.05)
            winkel, gesamt = G.Winkelmessen()
            alarm_HL, alarm_VL, alarm_HR, alarm_VR = P.prox_alarm()
        F.stop()
...
def einparken_l():
    ...
    W.write_Log(str(gesamt))
    W.write_Log(str(G.letzterwinkel))
